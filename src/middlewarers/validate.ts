...
const required_validator = (req: express.Request, res: express.Response, next: express.NextFunction): void =>{
...
const type_validator = (req: express.Request, res: express.Response, next: express.NextFunction): void =>{
  const invalidTypes: string[] = [];
  let error: boolean = false;
  const filename = ((typeof(req.query.filename) as unknown) as string === 'string') as boolean;
  const valid_width = !isNaN(parseInt((req.query.width as unknown) as string)) as boolean;
  const valid_height = !isNaN(parseInt((req.query.height as unknown) as string)) as boolean;
  ...
  if (req.query.blur){
      const blurCHeck = !isNaN(parseInt(((req.query.blur as unknown) as string).trim())) as boolean;
      if (!blurCHeck){
        invalidTypes.push('blur: number');
        error = true;
      } else {
        // save the value of blur in locals
        res.locals.blur = Math.round(parseInt(((req.query.blur as unknown) as string).trim()));
      }
  }
  // check optional params (Only allow true in rotate)
  if (req.query.rotate){
    const rotateCheck = !isNaN(parseInt(((req.query.rotate as unknown) as string).trim())) as boolean;
    if (!rotateCheck){
      invalidTypes.push('rotate: number');
      error = true;
    } else {
      res.locals.rotate = Math.round(parseInt(((req.query.rotate as unknown) as string).trim()));
    }
  }
